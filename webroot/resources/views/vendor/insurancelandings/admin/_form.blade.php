...
{!! BootForm::select(
        trans('validation.attributes.cover_card_blocks'),
        'coverCardBlocks[]',
        $model->coverCardBlocks->pluck('title', 'id')->all() + TypiCMS\Modules\Cardcoverblocks\Models\Cardcoverblock::all()->pluck('title', 'id')->all()
    )
    ->select($model->coverCardBlocks->pluck('id')->all())
    ->multiple(true)
    ->id('insuranceLandingCoverCardBlocks')
!!}
